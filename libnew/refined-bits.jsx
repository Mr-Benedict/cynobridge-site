...
function RSection({ children, action }) {
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
      <span style={{ fontFamily: FONT.display, fontSize: 12, fontWeight: 700, color: C.mfg, letterSpacing: "0.14em", textTransform: "uppercase" }}>{children}</span>
      {action}
    </div>
  );
}
...
function Figure({ label, value, unit, color, delta, deltaColor }) {
  return (
    <div style={{ minWidth: 0 }}>
      <div style={{ fontFamily: FONT.body, fontSize: 11, color: C.mfg, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 3 }}>{label}</div>
